export const getExploreNearby = async () => {
  // Static data for Mohali nearby locations
  // Future me yeh dynamic ho jayega
  return [
    {
      location: 'Chandigarh',
      img: '/assets/explore-nearby/1to.webp',
      distance: '15 km away',
    },
    {
      location: 'Panchkula',
      img: '/assets/explore-nearby/2k3.webp',
      distance: '12 km away',
    },
    {
      location: 'Zirakpur',
      img: '/assets/explore-nearby/40m.webp',
      distance: '18 km away',
    },
    {
      location: 'Kharar',
      img: '/assets/explore-nearby/41m.webp',
      distance: '22 km away',
    },
    {
      location: 'Kurali',
      img: '/assets/explore-nearby/5j2.webp',
      distance: '28 km away',
    },
    {
      location: 'Rupnagar',
      img: '/assets/explore-nearby/kji.webp',
      distance: '42 km away',
    },
    {
      location: 'Patiala',
      img: '/assets/explore-nearby/msp.webp',
      distance: '65 km away',
    },
    {
      location: 'Ambala',
      img: '/assets/explore-nearby/ynx.webp',
      distance: '55 km away',
    },
  ];
};

export const getLiveAnywhere = async () => {
  const liveAnywhereResponse = await fetch(
    'https://firebasestorage.googleapis.com/v0/b/edwintantawi-25f09.appspot.com/o/airbnb-web-clone%2Flive-anywhere.json?alt=media'
  );
  return liveAnywhereResponse.json();
};

// Pre-launch properties
export const getPreLaunchProperties = async () => {
  return [
    {
      id: 'noble-callista-flats',
      img: 'https://noble.sales-office.in/list/Noble%20Callista%20Flats.webp',
      title: 'Noble Callista Flats',
      description: 'Luxury Apartments - 3+1, 4+1 & 5+1 BHK, Earth Villas, Sky Villas | 2850/3500/5000 Sq. Ft.* Onwards | Near Airport | Under Construction | Rera Approved: PBRERA-SAS81-PR0748',
      location: 'Near Airport',
      price: '₹2.85 Cr*',
      star: 4.9,
      reviews: 120,
    },
    {
      id: 'noble-callista',
      img: 'https://noble.sales-office.in/list/Noble%20Callista.webp',
      title: 'Noble Callista',
      description: 'Luxury Residences - 3, 3+1, 4+1 & 5+1 BHK, Earth Villas, Sky Villas | 2400/2700/3200/4000/5300 Sq. Ft.* Onwards | Purb Apartments | Under Construction | Pre-Launch: 25% Payment Plan',
      location: 'Purb Apartments',
      price: '₹2.10 Cr*',
      star: 4.8,
      reviews: 95,
    },
    {
      id: 'nobel-flats-mohali',
      img: 'https://noble.sales-office.in/list/NOBEL-Flats-in-Mohali.webp',
      title: 'Nobel Flats in Mohali',
      description: 'Ultra Luxury Floors - 3 BHK + 3 W | 2051 Sq. Ft.* Onwards | Near Wave Estate | Nearing Possession | Rera Approved: PBRERA-SAS91-PR0854',
      location: 'Near Wave Estate, Mohali',
      price: '₹1.75 Cr*',
      star: 4.95,
      reviews: 78,
    },
    {
      id: 'noble-industrial-park',
      img: 'https://noble.sales-office.in/list/Noble-Callista-Flats.jpg',
      title: 'Noble Industrial Park',
      description: 'Industrial Plots - 500-3000 Sq.Yds.* Onwards | 9000 Sq.Yds* Onwards | Lalru Ambala-CHD Road | Under Development | Pre-Launch: 25% Payment Plan',
      location: 'Lalru Ambala-CHD Road',
      price: '₹9000 Sq.Yds*',
      star: 4.7,
      reviews: 45,
    },
  ];
};

// Location-wise property listings
export const getLocationListings = async () => {
  // Mock data for location-based sections
  // Future me yeh dynamic ho jayega based on location
  return {
    chandigarh: [
      {
        id: '1',
        img: '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
        location: 'Chandigarh',
        title: 'Room in Chandigarh',
        description: 'Centrally located private room',
        roomDetails: '1 king bed',
        star: 4.97,
        reviews: 240,
        price: '₹2,397',
        checkIn: '12 Dec',
        checkOut: '14 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        ],
      },
      {
        id: '2',
        img: '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        location: 'Chandigarh',
        title: 'Flat in Chandigarh',
        description: 'Modern apartment with city view',
        roomDetails: '1 bedroom',
        star: 4.95,
        reviews: 180,
        price: '₹3,995',
        checkIn: '15 Dec',
        checkOut: '17 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
          '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
        ],
      },
      {
        id: '3',
        img: '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
        location: 'Chandigarh',
        title: 'Home in Chandigarh',
        description: 'Spacious home with garden',
        roomDetails: '3 bedrooms',
        star: 4.98,
        reviews: 320,
        price: '₹4,537',
        checkIn: '20 Dec',
        checkOut: '22 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
          '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
        ],
      },
      {
        id: '4',
        img: '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
        location: 'Zirakpur',
        title: 'Apartment in Zirakpur',
        description: 'Modern apartment near highway',
        roomDetails: '2 bedrooms',
        star: 4.96,
        reviews: 156,
        price: '₹2,384',
        checkIn: '18 Dec',
        checkOut: '20 Dec',
        isGuestFavourite: false,
        images: [
          '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
          '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
        ],
      },
      {
        id: '5',
        img: '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
        location: 'Chandigarh',
        title: 'Room in Chandigarh',
        description: 'Cozy room with modern amenities',
        roomDetails: '1 double bed',
        star: 4.85,
        reviews: 92,
        price: '₹1,652',
        checkIn: '22 Dec',
        checkOut: '24 Dec',
        isGuestFavourite: false,
        images: [
          '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
          '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        ],
      },
      {
        id: '6',
        img: '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        location: 'Chandigarh',
        title: 'Room in Chandigarh',
        description: 'Comfortable stay in prime location',
        roomDetails: '1 king bed',
        star: 4.88,
        reviews: 145,
        price: '₹2,465',
        checkIn: '25 Dec',
        checkOut: '27 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        ],
      },
      {
        id: '7',
        img: '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
        location: 'Chandigarh',
        title: 'Room in Chandigarh',
        description: 'Modern room with park view',
        roomDetails: '1 queen bed',
        star: 4.91,
        reviews: 210,
        price: '₹2,966',
        checkIn: '28 Dec',
        checkOut: '30 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        ],
      },
    ],
    gurgaon: [
      {
        id: '8',
        img: '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        location: 'Gurugram',
        title: 'Flat in Gurugram',
        description: 'Luxury flat with city view',
        roomDetails: '2 bedrooms',
        star: 5.0,
        reviews: 280,
        price: '₹2,454',
        checkIn: '10 Dec',
        checkOut: '12 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
          '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
        ],
      },
      {
        id: '9',
        img: '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
        location: 'Gurugram',
        title: 'Flat in Gurugram',
        description: 'Modern apartment with balcony',
        roomDetails: '1 bedroom',
        star: 4.98,
        reviews: 195,
        price: '₹3,709',
        checkIn: '15 Dec',
        checkOut: '17 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
          '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
        ],
      },
      {
        id: '10',
        img: '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
        location: 'Gurugram',
        title: 'Flat in Gurugram',
        description: 'Cozy flat in prime location',
        roomDetails: '1 bedroom',
        star: 4.8,
        reviews: 120,
        price: '₹1,699',
        checkIn: '20 Dec',
        checkOut: '22 Dec',
        isGuestFavourite: false,
        images: [
          '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
          '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
        ],
      },
      {
        id: '11',
        img: '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
        location: 'Sushant Lok',
        title: 'Flat in Sushant Lok',
        description: 'Spacious flat with modern amenities',
        roomDetails: '2 bedrooms',
        star: 4.95,
        reviews: 165,
        price: '₹2,049',
        checkIn: '18 Dec',
        checkOut: '20 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
          '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        ],
      },
      {
        id: '12',
        img: '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        location: 'Gurugram',
        title: 'Flat in Gurugram',
        description: 'Luxury apartment with city view',
        roomDetails: '3 bedrooms',
        star: 5.0,
        reviews: 310,
        price: '₹3,681',
        checkIn: '22 Dec',
        checkOut: '24 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
        ],
      },
      {
        id: '13',
        img: '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
        location: 'Sector 43',
        title: 'Flat in Sector 43',
        description: 'Modern flat in gated community',
        roomDetails: '2 bedrooms',
        star: 4.84,
        reviews: 98,
        price: '₹3,100',
        checkIn: '25 Dec',
        checkOut: '27 Dec',
        isGuestFavourite: false,
        images: [
          '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        ],
      },
      {
        id: '14',
        img: '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
        location: 'Gurugram',
        title: 'Home in Gurugram',
        description: 'Spacious home with garden',
        roomDetails: '4 bedrooms',
        star: 5.0,
        reviews: 245,
        price: '₹2,910',
        checkIn: '28 Dec',
        checkOut: '30 Dec',
        isGuestFavourite: true,
        images: [
          '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
          '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
        ],
      },
    ],
  };
};

export const getSearch = async () => {
  // Static data for search results - Mohali/Chandigarh area listings
  // Future me yeh dynamic ho jayega based on location
  return [
    {
      id: '1',
      img: '/assets/search/013c9377-349f-418b-8d4c-15f923234a5f.webp',
      location: 'Sahibzada Ajit Singh Nagar',
      title: 'Room in Sahibzada Ajit Singh Nagar',
      description: 'BlueBliss(bath...',
      roomDetails: '1 king bed',
      star: '4.89',
      reviews: '103',
      price: '₹2,511/night',
      lat: 30.7046,
      long: 76.7179,
      checkIn: '20 Dec',
      checkOut: '25 Dec',
      isGuestFavourite: true,
    },
    {
      id: '2',
      img: '/assets/search/1379331e-593a-4c1e-af51-222808c85a11.webp',
      location: 'Chandigarh',
      title: 'Centrally located private room with a park view',
      description: 'Centrally located private room',
      roomDetails: '1 king bed',
      star: '4.97',
      reviews: '240',
      price: '₹2,397/night',
      lat: 30.7333,
      long: 76.7794,
      checkIn: '12 Dec',
      checkOut: '17 Dec',
      isGuestFavourite: true,
    },
    {
      id: '3',
      img: '/assets/search/2dd686bc-0195-40db-a37f-8b02476415b7.webp',
      location: 'Chandigarh',
      title: 'Apartment in Chandigarh',
      description: 'Subko Abode- RK with Mountain...',
      roomDetails: '1 bedroom 1 double bed',
      star: '5.0',
      reviews: '5',
      price: '₹2,876/night',
      lat: 30.7415,
      long: 76.7681,
      checkIn: '10 Dec',
      checkOut: '15 Dec',
      isGuestFavourite: false,
    },
    {
      id: '4',
      img: '/assets/search/44cb0de7-fa62-49e2-b4b8-68aed14373cb.webp',
      location: 'Panchkula',
      title: 'Room in Panchkula',
      description: 'Cozy room with modern amenities',
      roomDetails: '1 queen bed',
      star: '4.85',
      reviews: '87',
      price: '₹2,055/night',
      lat: 30.6942,
      long: 76.8616,
      checkIn: '18 Dec',
      checkOut: '23 Dec',
      isGuestFavourite: true,
    },
    {
      id: '5',
      img: '/assets/search/97bc37a6-9a1b-4bb2-8564-771319b246fb.webp',
      location: 'Zirakpur',
      title: 'Apartment in Zirakpur',
      description: 'Spacious apartment near highway',
      roomDetails: '2 bedrooms',
      star: '4.92',
      reviews: '156',
      price: '₹3,200/night',
      lat: 30.6422,
      long: 76.8172,
      checkIn: '15 Dec',
      checkOut: '20 Dec',
      isGuestFavourite: false,
    },
    {
      id: '6',
      img: '/assets/search/dde44668-1df5-41b6-8f91-5051975c4865.webp',
      location: 'Mohali',
      title: 'Room in Mohali',
      description: 'Comfortable stay near airport',
      roomDetails: '1 double bed',
      star: '4.78',
      reviews: '92',
      price: '₹1,898/night',
      lat: 30.7046,
      long: 76.7179,
      checkIn: '22 Dec',
      checkOut: '27 Dec',
      isGuestFavourite: true,
    },
  ];
};
